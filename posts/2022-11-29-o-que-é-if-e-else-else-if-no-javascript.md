---
title: O que é if e else else if no javascript?
description: if e else são estruturas de controle de fluxo que permitem que você
  execute um bloco de código se uma condição for verdadeira e outro bloco de
  código se a condição for falsa.
date: 2022-11-29 01:35:03
thumbnail: data:image/png;base64,
category: Javascript
background: "#FEC748"
---
if em português significa se, e else significa senão.

else if é uma condição que pode ser adicionada a um if ou else if anterior, para que você possa verificar várias condições e executar um bloco de código específico para cada condição.

```javascript
// Exemplo 1

var idade1 = 18; // variável idade recebe o valor 18

if (idade1 >= 18) {
  // se a idade for maior ou igual a 18
  console.log("Você é maior de idade"); // imprime no console "Você é maior de idade"
} else {
  // senão
  console.log("Você é menor de idade"); // imprime no console "Você é menor de idade"
} // fim do if

// <---Saída--->
// Você é maior de idade

// Exemplo 2

var idade2 = 17; // variável idade recebe o valor 18

if (idade2 >= 18) {
  // se a idade for maior ou igual a 18
  console.log("Você é maior de idade"); // imprime no console "Você é maior de idade"
} else if (idade2 >= 16) {
  // senão se a idade for maior ou igual a 16
  console.log("Você pode votar"); // imprime no console "Você pode votar"
} else {
  // senão
  console.log("Você é menor de idade"); // imprime no console "Você é menor de idade"
} // fim do if

// <---Saída--->
// Você pode votar
```