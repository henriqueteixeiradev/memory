---
title: o que é try catch no javascript?
description: try catch é uma estrutura de controle que permite que você trate
  erros em seu código javascript
date: 2022-11-29 01:22:13
...
background: "#FEC748"
---
try: é o bloco de código que você quer testar

catch: é o bloco de código que você quer executar se o bloco try falhar

try: em português significa tentar

catch: em português significa pegar

```javascript
function testandoTryCatch(nome) {
  if (nome == undefined) {
    throw new Error("O nome não foi passado");
  }
}

try {
  console.log(testandoTryCatch());
} catch (error) {
  console.log(error + " - erro o nome deve ser passado");
}

// <---Saída--->
// Error: O nome não foi passado - erro o nome deve ser passado
```