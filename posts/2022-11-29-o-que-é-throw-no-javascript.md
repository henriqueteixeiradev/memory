---
title: O que é throw no javascript?
description: throw é uma palavra reservada do javascript que lança uma exceção,
  ou seja, um erro.
date: 2022-11-29 12:58:18
thumbnail: data:image/png;base64,
category: Typescript
background: "#0076c6"
---
```javascript
function sayMyName(name = "") {
  if (name === "") {
    // se o nome for vazio
    throw "Nome é obrigatório"; // lança um erro
  }
  console.log("depois do erro"); // não será executado
}

console.log(sayMyName("")); // lança um erro

// <---Saída--->
// c:\Users\rick1\OneDrive\Área de Trabalho\teste\script.js:9
//     throw "Nome é obrigatório"; // lança um erro
//     ^
// Nome é obrigatório
// (Use `node --trace-uncaught ...` to show where the exception was thrown)

```