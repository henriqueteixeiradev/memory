---
title: O que é controle de fluxo de aplicação no javascript?
description: O controle de fluxo de aplicação é o que permite que o código seja
  executado de acordo com a ordem que você deseja.
date: 2022-11-29 01:42:10
thumbnail: data:image/png;base64,
category: Javascript
background: "#FEC748"
---
Por exemplo, se você quiser que o código seja executado apenas se uma condição for verdadeira,

você pode usar um if statement.

Se você quiser que o código seja executado várias vezes, você pode usar um loop.

Se você quiser que o código seja executado várias vezes, mas apenas se uma condição for verdadeira,

você pode usar um loop dentro de um if statement.

```javascript
// O que é um if statement?
// Um if statement é uma declaração que permite que o código seja executado apenas se uma condição for verdadeira.

// Exemplo de if statement

if (condição) {
  // código a ser executado se a condição for verdadeira
} else {
  // código a ser executado se a condição for falsa
} // fim do if statement
```