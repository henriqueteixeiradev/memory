---
title: while
description: A declaração while cria um laço que executa uma rotina especifica
  enquanto a condição de teste for avaliada como verdadeira. A condição é
  avaliada antes da execução da rotina.
date: 2022-11-29 12:50:52
thumbnail: data:image/png;base64,
category: Typescript
background: "#0076c6"
---
```javascript
// Wile

let i = 0; // Inicialização

// Condição
while (i < 10) {
  console.log("Hello, World!" + " " + i); // Execução
  i++; // Incremento
} // Fim do loop

// <---Saída--->
// Hello, World! 0
// Hello, World! 1
// Hello, World! 2
// Hello, World! 3
// Hello, World! 4
// Hello, World! 5
// Hello, World! 6
// Hello, World! 7
// Hello, World! 8
// Hello, World! 9
```